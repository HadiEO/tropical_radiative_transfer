...
#  if(MCmethod == "sobolEff") x0 <- sobolEff(model=NULL, X1, X2, ...)
  ...
  if (!is.null(model)){
    y <- model(x0$X, ...)                                                                  # What is y? What does this line do?                                                       

    V <- 0
    foreach (i in 1:q) %dopar% {                                                           # foreach () %dopar% {}
      tell(x0,y[,i])                                                                       # Model is passed to sobol samples
      V <- V + x0$V[1]
    ...
    x <- x0
    x$V[1] <- V
    foreach (i in 1:p) %dopar% {                                                           # foreach () %dopar% {}
